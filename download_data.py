    ...
    try:
        with open('cookie_info') as json_file:
            try:
                ...
                return datetime.strptime(created_on, '%Y-%m-%d %H:%M:%S')
            except:
                return 0
    except:
        return 0
...
class bulk_downloader:
    def __init__(self, id, username, password):
        ...
        if id[:3] == 'S1A':
            ...
            self.files = ['https://datapool.asf.alaska.edu/GRD_HD/SB/{}.zip'.format(id)]
        else:
